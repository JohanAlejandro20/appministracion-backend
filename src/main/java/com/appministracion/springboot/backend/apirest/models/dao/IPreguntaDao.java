...
public interface IPreguntaDao extends CrudRepository<Pregunta, Long> {
    ...
    @Query(value = "select p.cod_pregunta as codigo_pregunta, u.nombre as nombre_usuario, p.nombre as nombre_pregunta, p.descripcion as descripcion_pregunta, p.create_at  as fecha_creacion_pregunta, c.nombre as nombre_conjunto, r.cod_respuesta as codigo_respuesta from \n"
    		+ "usuarios u inner join preguntas p on p.cod_usuario = u.cod_usuario\n"
    		+ "inner join conjuntos c on c.cod_conjunto = u.cod_conjunto  left join respuestas r on r.cod_pregunta  = p.cod_pregunta where u.cod_conjunto = :id_conjunto and r.cod_respuesta is not null", nativeQuery = true )  
    public List<Map<String,Object>> findQuestionByConjuntoWithResponse(long id_conjunto);
    
    
    @Query(value = "select p.cod_pregunta as codigo_pregunta, u.nombre as nombre_usuario, p.nombre as nombre_pregunta, p.descripcion as descripcion_pregunta, p.create_at  as fecha_creacion_pregunta, c.nombre as nombre_conjunto, r.cod_respuesta as codigo_respuesta from \n"
    		+ "usuarios u inner join preguntas p on p.cod_usuario = u.cod_usuario\n"
    		+ "inner join conjuntos c on c.cod_conjunto = u.cod_conjunto  left join respuestas r on r.cod_pregunta  = p.cod_pregunta where u.cod_conjunto = :id_conjunto and r.cod_respuesta is null", nativeQuery = true )  
